...
foreach ( $logos as $slug => $l ) {
	...
	$inner .= '<figure class="wsg-card wsg-logo-card"><div class="wsg-logo-stages' . ( 1 === count( $stages ) ? ' is-single' : '' ) . '">' . $panels . '</div><figcaption class="wsg-meta"><span class="wsg-name">' . esc_html( $l['label'] ) . '</span>' . ( ! empty( $l['url'] ) ? ' <a class="wsg-small" href="' . esc_url( $l['url'] ) . '" download>' . esc_html__( 'Download', 'weave-style-guide' ) . '</a>' : '' ) . '</figcaption></figure>';
}
if ( $icon ) {
